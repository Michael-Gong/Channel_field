...
##end for norm colorbar####
  ...
  part13_id = part13_id[ (gg>100.0) & (work_x > 5*work_y) ]
...
#  np.savetxt(to_path+'xx_2d_x.txt',xx_2d)
#  np.savetxt(to_path+'yy_2d_x.txt',yy_2d)
#  np.savetxt(to_path+'gg_2d_x.txt',gg_2d)
  ...
  gg = (px**2+py**2+1)**0.5
  part13_id = data['Particles/ID/subset_high_e/electron'].data
  part13_id = part13_id[ (gg>150.0) & (work_x*5 < work_y) ]
  print('part13_id size is ',part13_id.size,' max ',np.max(part13_id),' min ',np.min(part13_id))
  ...
  for n in range(start,stop+step,step):
      ...
      for ie in range(part_id.size):
          ...
          #work_y_2d[ie,n-start] = work_y[temp_id==part_id[ie]]
      print('finised constructing 2darray'+str(round(100.0*(n-start+step)/(stop-start+step),4))+'%')
  np.savetxt(to_path+'xx_2d_y.txt',xx_2d)
  np.savetxt(to_path+'yy_2d_y.txt',yy_2d)
  np.savetxt(to_path+'gg_2d_y.txt',gg_2d)
