...
##end for norm colorbar####
  ...
  xx_2d = np.loadtxt('./jpg_shining/xx_2d.txt') 
  yy_2d = np.loadtxt('./jpg_shining/yy_2d.txt') 
  gg_2d = np.loadtxt('./jpg_shining/gg_2d.txt') 
...
#  youwant = ['electron_x_px','electron_density','electron_en','electron_theta_en','ey'] #,'electron_ekbar']
  ...
  for n in range(start,stop+step,step):
    ...
    if n-50 >= 6:
        ...
        select = np.where(gg_2d[:,n-50] > 5.0)
        print('select is',select)
        print('xx_2d[:,index][select,:] shape is',xx_2d[:,index][select,:].shape)
        #print('yy_2d[select,index] shape is',(yy_2d[select,index]).shape)
        #print('xx_2d[select,0].size is',xx_2d[select,0].size)
        plt.scatter(xx_2d[:,index][select,:].reshape(np.array(select).size,index.size), yy_2d[:,index][select,:].reshape(np.array(select).size,index.size), c=np.tile(np.arange(index.size)*5, (xx_2d[select,0].size, 1)), s=np.tile(np.arange(index.size)*5, (xx_2d[select,0].size, 1)), cmap='GnBu', edgecolors='None')
    plt.ylim(-5,5)
    plt.xlim(0,30)
