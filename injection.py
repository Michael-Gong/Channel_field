...
work_x_2d = np.zeros([part_id.size,(stop-start)//step+1])
work_y_2d = np.zeros([part_id.size,(stop-start)//step+1])
for n in range(start,stop+step,step):
    #### header data ####
    data = sdf.read("./Data/"+str(n).zfill(4)+".sdf",dict=True)
    px = data['Particles/Px/subset_high_e/electron'].data/(m0*v0)
    ...
    for ie in range(part_id.size):
        ...
        work_x_2d[ie,(n-start)//step] = work_x[temp_id==part_id[ie]]
        work_y_2d[ie,(n-start)//step] = work_y[temp_id==part_id[ie]]
    print('finised '+str(round(100.0*(n-start+step)/(stop-start+step),4))+'%')

outputdir = './inject_txt/'

np.savetxt(outputdir+'px2d.txt',px_2d)
np.savetxt(outputdir+'py2d.txt',py_2d)
np.savetxt(outputdir+'xx2d.txt',xx_2d)
np.savetxt(outputdir+'yy2d.txt',yy_2d)
np.savetxt(outputdir+'workx2d.txt',work_x_2d)
np.savetxt(outputdir+'worky2d.txt',work_y_2d)
