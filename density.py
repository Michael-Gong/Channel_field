...
##end for transparent colorbar##
  ...
  for n in range(start,stop+step,step):
    ...
    for name in ['charge_dens']:
        ...
        den_e = den_e[:,200:400]
        den_i = data['Derived/Number_Density/carbon'].data/denunit
        den_i = den_i[:,200:400]
        den =den_e
        #den = -den_e + den_i
        ...
        cbar.set_label('electron [$n_c$]', fontdict=font)
        ...
        fig.savefig(to_path+'electron_den'+str(n).zfill(4)+'.png',format='png',dpi=80)
        plt.close("all")
        ...
        #cbar=plt.colorbar(ticks=np.linspace(-20.0, 20.0, 5))
        cbar=plt.colorbar(ticks=np.logspace(-2,2,4))
        cbar.set_label('carbon [$n_c$]', fontdict=font)
        plt.xlabel('X [$\lambda_0$]',fontdict=font)
        plt.ylabel('Y [$\lambda_0$]',fontdict=font)
        plt.xticks(fontsize=20); plt.yticks(fontsize=20);
        plt.title('carbon at '+str(round(time/1.0e-15,6))+' fs',fontdict=font)
        fig = plt.gcf()
        fig.set_size_inches(12, 7)
        fig.savefig(to_path+'carbon_den'+str(n).zfill(4)+'.png',format='png',dpi=80)
        plt.close("all")
