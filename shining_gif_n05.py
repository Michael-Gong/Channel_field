...
##end for norm colorbar####
  ...
  start   =  250  # start time
  stop    =  350  # end time
  step    =  1  # the interval or step
  from_path='./Data_n05_fine/'
  from_path1='./jpg_n05_fine/'
  to_path  ='./jpg_n05_fine/'
...
#  youwant = ['electron_x_px','electron_density','electron_en','electron_theta_en','ey'] #,'electron_ekbar']
  ...
  for n in range(start,stop+step,step):
    ...
    den_p = data['Derived/Number_Density/electron_outside_no'].data/denunit
    den = den+den_p
    den = den[:,:]
    ...
    if n-1 >= 6:
        print('n-1=',n-1)
        index=np.arange(n-1-6,n-1,1)
        print('index is',index)
        select = np.where(gg_2d_outer[:,n-1] > 1.3)
        print('select is',select)
        print('xx_2d_outer[:,index][select,:] shape is',xx_2d_outer[:,index][select,:].shape)
        #print('yy_2d_x[select,index] shape is',(yy_2d_x[select,index]).shape)
        #print('xx_2d_x[select,0].size is',xx_2d_x[select,0].size)
        plt.scatter(xx_2d_outer[:,index][select,:].reshape(np.array(select).size,index.size), yy_2d_outer[:,index][select,:].reshape(np.array(select).size,index.size), c=np.tile(np.arange(index.size)*5, (xx_2d_outer[select,0].size, 1)), s=np.tile(np.arange(index.size)*5, (xx_2d_outer[select,0].size, 1)), cmap='GnBu', edgecolors='None')
        select = np.where(gg_2d_iner[:,n-1] > 1.3)
        print('select is',select)
        print('xx_2d_iner[:,index][select,:] shape is',xx_2d_iner[:,index][select,:].shape)
        #print('yy_2d_y[select,index] shape is',(yy_2d_y[select,index]).shape)
        #print('xx_2d_y[select,0].size is',xx_2d_y[select,0].size)
        plt.scatter(xx_2d_iner[:,index][select,:].reshape(np.array(select).size,index.size), yy_2d_iner[:,index][select,:].reshape(np.array(select).size,index.size), c=np.tile(np.arange(index.size)*5, (xx_2d_iner[select,0].size, 1)), s=np.tile(np.arange(index.size)*5, (xx_2d_iner[select,0].size, 1)), cmap='rainbow', edgecolors='None')
    plt.ylim(-5,5)
   ...
   # plt.xlim(0,30)
    ...
    fig.savefig(to_path+str(n).zfill(4)+'.png',format='png',dpi=80)
    plt.close("all")
