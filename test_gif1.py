...
#  youwant = ['electron_x_px','electron_density','electron_en','electron_theta_en','ey'] #,'electron_ekbar']
  ...
  for n in range(start,stop+step,step):
    #### header data ####
    data = sdf.read("./Data_new/"+str(n).zfill(4)+".sdf",dict=True)
    header=data['Header']
    time=header['time']
    ...
    if np.min(ex.T) == np.max(ex.T):
            continue
    levels = np.linspace(-22.5, 22.5, 101)
    ...
    if 'Particles/Px/subset_high_e/electron' in data:
        ...
        if gg.size > 1.5:
            ...
            plt.scatter(ppp_x, ppp_y, s=20, c='green', norm=mcolors.Normalize(vmin=levels.min(), vmax=levels.max()), edgecolors='None', alpha=0.3)
    ...
    fig.savefig('./fig_new/gif_'+str(n).zfill(4)+'.png',format='png',dpi=80)
    plt.close("all")
