...
#  youwant = ['electron_x_px','electron_density','electron_en','electron_theta_en','ey'] #,'electron_ekbar']
  ...
  for n in range(start,stop+step,step):
    ...
    if n-50 >= 6:
        ...
        plt.scatter(xx_2d_x[:,index][select,:].reshape(np.array(select).size,index.size), yy_2d_x[:,index][select,:].reshape(np.array(select).size,index.size), c=np.tile(np.arange(index.size)*5, (xx_2d_x[select,0].size, 1)), s=np.tile(np.arange(index.size)*5, (xx_2d_x[select,0].size, 1)), cmap='GnBu', edgecolors='None')
        ...
        plt.scatter(xx_2d_y[:,index][select,:].reshape(np.array(select).size,index.size), yy_2d_y[:,index][select,:].reshape(np.array(select).size,index.size), c=np.tile(np.arange(index.size)*5, (xx_2d_y[select,0].size, 1)), s=np.tile(np.arange(index.size)*5, (xx_2d_y[select,0].size, 1)), cmap='OrRd', edgecolors='None')
    plt.ylim(-5,5)
